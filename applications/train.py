...
def train_epoch(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, scores, _) in enumerate(train_loader):
        ...
        #-----------------------------zhengruidi-------------------#
        #logging.info(outputs.view(-1).sigmoid()[0::2])
        #logging.info(outputs.view(-1).sigmoid()[1::2])
        #Num_Patch = 30
        #Num_Image = images.shape[0]
        #outputs = np.zeros(Num_Image)
        #imgs = transforms.ToPILImage()(images[0])
        #imgs.show()
        #for j in range(Num_Patch):
        #    images_ = transforms.RandomResizedCrop(224)(images)
        #    imgs_ = transforms.ToPILImage()(images_[0])
        #    imgs_.show()
        #    #print(imgs)
         #   #print(imgs_)
